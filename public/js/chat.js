...
function registerSocketEvents() {
  ...
  s.on('message-deleted', (data) => {
    ...
    if (Chat.chatMessages[chatId]) {
      const idx = Chat.chatMessages[chatId].findIndex(m => m.id === messageId);
      if (idx >= 0) Chat.chatMessages[chatId].splice(idx, 1);
    }
  });
...
Object.assign(Chat, {
  ...
  // 语音录制功能见上方完整实现（toggleVoiceRecording/startRecording/stopRecording/sendVoiceMessage）
  ...
  async showChatInfoPanel() {
    ...
    document.getElementById('btn-clear-chat-history')?.addEventListener('click', () => {
      ...
      UI.showChoice('清空聊天记录', '选择清空方式', [
        { text: '仅清空我设备上的', value: 'for_me', class: 'btn-secondary' },
        { text: '同时清空对方设备上的', value: 'for_all', class: 'btn-danger' }
      ], async (scope) => {
        try {
          const resp = await fetch('/api/chats/' + this.currentChat.id + '/messages', {
            method: 'DELETE',
            headers: { 'Content-Type': 'application/json' },
          ...
          if (data.success) {
            ...
            UI.showToast(scope === 'for_all' ? '双方聊天记录已清空' : '我设备聊天记录已清空');
            panel.classList.add('hidden');
          } else {
...
Object.assign(Chat, {
  showMessageMenu(messageEl, message, isSent) {
    ...
    // 多选模式
    menuItems.push({ icon: "☑️", text: "多选", action: () => this.enterMultiSelectMode(message) });
  ...
  async deleteMessage(messageEl, message) {
    const userId = Auth.getCurrentUserId();
    UI.showChoice('删除消息', '选择删除方式', [
      { text: '仅删我设备上的', value: 'for_me', class: 'btn-secondary' },
      { text: '同时删对方设备上的', value: 'for_all', class: 'btn-danger' }
    ], async (scope) => {
      try {
        ...
        UI.showToast(scope === 'for_all' ? '消息已从双方设备删除' : '消息已从我设备删除');
      } catch (e) {
